...
From the project root directory, compile all Java source files into an `out/` or `bin/` directory:
...
Unit tests are located under the `test/` directory. If using JUnit 4/5:
...
*Tip: You can also open the project in IntelliJ IDEA or Eclipse and run all test cases with a single click.*
...
Here is the generated `README.md` content based on the project structure:

```markdown
# Maritime Traffic Simulator (Simulador Marítimo)

A Java-based desktop application designed for simulating, managing, and visualizing maritime traffic in customizable environments[cite: 1]. The simulator models ship navigation, route optimization using pathfinding algorithms (such as Dijkstra's algorithm), dynamic obstacle avoidance, environmental conditions (e.g., storms and currents), and harbor/traffic control operations[cite: 1].
...
- **Maritime Navigation & Autopilot:**
  - Dynamic simulation of vessels (`Navio`, `Navegante`) moving across geometric coordinates[cite: 1].
  - State management for vessels (`EstadoMovel`: *Waiting*, *At Origin*, *Navigating*, *At Destination*)[cite: 1].
  - Autopilot support for path tracking and vector computations[cite: 1].

- **Route Planning & Graph Routing:**
  - Route calculation using graph-based pathfinding strategies (`EstrategiaDijkstra`, `Grafo`, `Route`)[cite: 1].
  - Strategy Pattern implementation (`EstrategiaRota`) for modular routing algorithms[cite: 1].

- **Obstacles & Environmental Dynamics:**
  - Geometric collision boundaries and obstacles (`Obstaculo`, `Circulo`, `Poligono`, `Retangulo`, `Quadrado`, `Triangulo`)[cite: 1].
  - Environmental events such as ocean currents (`DialogoCorrente`) and moving storms (`Tempestade`)[cite: 1].

- **Harbor & Control Operations:**
  - Harbor management (`Porto`) and traffic control tower (`TorreDeControlo`, `GestorMaritimo`)[cite: 1].
  - State persistence and snapshot capture (`SnapshotSimulacao`) for tracking or replaying simulations[cite: 1].

- **Interactive GUI (Java Swing):**
  - Real-time 2D map visualization (`PainelMapa`, `JanelaPrincipal`)[cite: 1].
  - Interactive simulation controls: play/pause, restart (`AcaoReiniciarSimulacao`), toggle vessel direction (`AcaoAlternarDirecao`), and configure ocean currents[cite: 1].

- **Comprehensive Unit Testing:**
  - Extensive test coverage for engine components, vector math, geometry, and graph routing algorithms[cite: 1].

---

## 🏗 Architecture & Design Patterns

The project follows a clean separation of concerns, divided into three main packages[cite: 1]:
...
### Key Design Patterns Used:
- **Strategy Pattern:** `EstrategiaRota` interface implemented by `EstrategiaDijkstra` to swap routing strategies cleanly[cite: 1].
- **State Pattern:** `EstadoMovel` hierarchy (`MovelNaOrigem`, `MovelNavegando`, `MovelAguardando`, `MovelNoDestino`) managing vessel lifecycle[cite: 1].
- **Memento / Snapshot Pattern:** `SnapshotSimulacao` for simulation state capture and playback[cite: 1].
- **MVC / Separation of Concerns:** Decoupled simulation logic (`Engine`) and graphical rendering (`GUI`)[cite: 1].
